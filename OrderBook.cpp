...
#include <map>
	...
	std::string OrderBook::getNextTime(std::string timestamp)
	{
		...
		if (next_timestamp == "")
		{
			...
			try {
				std::cout << "Cannot get next value." << std::endl;
			}
	...
	std::string OrderBook::getPrevTime(std::string timestamp)
	{
		...
		if (prev_timestamp == "")
		{
			...
			try {
				std::cout << "Cannot get previous value." << std::endl;
			}
			catch (const std::exception& e) {
...
//compare timestamps from the last to the first 
void OrderBook::backwardsOrder(OrderBookEntry& order)
{
	orders.push_back(order);
	std::sort(orders.begin(), orders.end(), OrderBookEntry::timestampDesc);
}